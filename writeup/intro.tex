...
\begin{document}
    ...
    \end{lstlisting}
    This entity describes the roles that are allowed for each permission evel. For each role there are different permissions that are dependent on a true or false value (BIT) in SQL. For each user the permission level determines what roles they have.
    \\
